...
resource "aws_instance" "test-instance" {
  ...
  provisioner "ucd" {
    agent_name      = "${var.test-instance_agent_name}.${random_id.test-instance_agent_id.dec}"
    ucd_server_url  = "${var.ucd_server_url}"
  ...
  provisioner "local-exec" {
    when = "destroy"
    command = <<EOT
    curl -k -u ${var.ucd_user}:${var.ucd_password} ${var.ucd_server_url}/cli/agentCLI?agent=${var.test-instance_agent_name}.${random_id.test-instance_agent_id.dec} -X DELETE
EOT
}
  tags {
...
resource "ucd_component_mapping" "ucd_blueprint_designer_test" {
  ...
  parent_id = "${ucd_agent_mapping.test-instance_agent.id}"
}
...
resource "ucd_agent_mapping" "test-instance_agent" {
  depends_on = [ "aws_instance.test-instance" ]
  description = "Agent to manage the test-instance server"
  agent_name = "${var.test-instance_agent_name}.${random_id.test-instance_agent_id.dec}"
